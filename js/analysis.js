...
let _mistakeRafId    = null;     // 実行待ちチェーンの rAF トークン（重複チェーン防止用）
...
function toggleMistakeList() {
  ...
  if (btn) btn.classList.toggle('active', showMistakeList);
...
function computeMistakes() {
  ...
  // 前のチェーンがまだ rAF 待ちなら取り消す（高速トグル時に複数チェーンが並走するのを防ぐ）
  if (_mistakeRafId !== null) {
    cancelAnimationFrame(_mistakeRafId);
    _mistakeRafId = null;
  }
  ...
  function processNext() {
    ...
    try { processNextImpl(); } catch (e) {
      console.error('悪手解析エラー:', e);
      // エラー発生時はこの世代をキャンセル扱いにして後続タスクを止める
      if (gen === mistakeGeneration) mistakeGeneration++;
    }
  }

  function processNextImpl() {
    if (gen !== mistakeGeneration) return; // try 外に出た場合の二重チェック

    // 新しい局面の初期化
    if (validMoves === null) {
      if (idx >= moveHistory.length) {
        // 全手の評価完了 → キャッシュ保存してレンダリング（上限超過分は古い順に削除）
        if (mistakeCacheMap.size >= MAX_MISTAKE_CACHE_ENTRIES) {
          mistakeCacheMap.delete(mistakeCacheMap.keys().next().value);
        }
  ...
  _mistakeRafId = requestAnimationFrame(() => { _mistakeRafId = null; processNext(); });
}
